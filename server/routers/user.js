// Delete file if you don't have user profiles/data in database
const express = require('express')
const User = require('../models/User')
const Example = require('../models/Example')
const jwt = require('jsonwebtoken')
const bcrypt = require('bcryptjs')
const { sendWelcomeEmail, sendChangePasswordEmail, sendCancelationEmail, sendUsEmail } = require('../emails/example')
...
router.post('/delete-user', async (req, res) => {
    ...
    try {
        ...
        const encodedCorrectCode = await bcrypt.hash(sendCancelationEmail({ email: user.email }) + process.env.BCRYPT_SECRET, 8)
...
router.post('/get-user', async (req, res) => {
    ...
    try {
        ...
        res.status(202).send({ user: { email: user.email, credit: user.credit }, Examples: madeExamples })
    }catch(err) {
        res.status(500).send()
    }
})
...
router.post('/send-email', (req, res) => {
    ...
    try {
        sendUsEmail({ email, subject, content })
        res.status(200).send()
    }catch(err) {
        res.status(500).send()
    }
...
router.post('/signup', async (req, res) => {
    ...
    try {
        ...
        const encodedCorrectCode = await bcrypt.hash(sendWelcomeEmail({ email }) + process.env.BCRYPT_SECRET, 8)
        res.status(202).send({ encodedCorrectCode })
    }catch(err) {
        res.status(500).send()
    }
...
router.post('/verify-delete-user', async (req, res) => {
    ...
    try {
        ...
        for(let i = 0; i < user.madeExamples.length; i++) {
            let Example = await Example.findById(user.madeExamples[i])
            if(Example) {
                const users = User.find({ subbedExample: Example._id })
                for(let j = 0; j < users.length; j++) {
                    users[i].record = []
                    users[i].subbedExample = null
                    users[i].save()
                }
...
router.post('/verify-user', async (req, res) => {
    const { email, password, code, encodedCorrectCode } = req.body
